...
public class LoginActivity extends AppCompatActivity {
    ...
    private void attemptLogin() {
        ...
        Integer tipoDocumento = mTipoDocumentoView.getSelectedItemPosition() + 1;
        String Documento = mDocumentoView.getText().toString();
        String login = "http://www.tucitaacin.com/autenticacion/recurso/usuarios.php";
        ...
        if (cancel) {
            ...
            try {
                ...
                if (networkInfo != null && networkInfo.isConnected()) {
                    //Toast.makeText(LoginActivity.this, "tipodocumento="+String.valueOf(tipoDocumento)+"&documento=" + Documento, Toast.LENGTH_LONG).show();
                    new UserLoginTask(String.valueOf(tipoDocumento), Documento, login).execute();
                } else {
     ...
     * the user.
     */
    public class UserLoginTask extends AsyncTask<String, Void, Login> {
        ...
        protected Login doInBackground(String... params) {
            ...
            String request = "tipo=" + tipodocumento + "&documento=" + documento;
            URL login = null;
            HttpURLConnection con = null;
            Login response = null;
            try {
                login = new URL(this.login);
            ...
            try {
                ...
                if (statusCode != 200) {
                    Gson gson = new Gson();
                    JsonReader reader = new JsonReader(new InputStreamReader(new BufferedInputStream(con.getInputStream()), "UTF-8"));
                    JsonParser parser = new JsonParser();
                    JsonElement jsonResponse = parser.parse(String.valueOf(reader));

                    try {
                        response = gson.fromJson(jsonResponse, Login.class);
